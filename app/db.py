...
def init_db(db_path: str) -> None:
    ...
    with _get_conn(db_path) as conn:
        ...
        for col in ["bluesky_posted_at", "tumblr_posted_at", "slickdeals_posted_at", "reddit_posted_at"]:
            try:
                if USE_POSTGRES:
...
def get_unreddited_products(db_path: str, limit: int = 10) -> list[dict]:
    ph = "%s" if USE_POSTGRES else "?"
    with _get_conn(db_path) as conn:
        return _rows(conn,
            f"SELECT * FROM products WHERE reddit_posted_at IS NULL AND price IS NOT NULL ORDER BY savings_percent DESC NULLS LAST LIMIT {ph}",
            (limit,))


def mark_reddit_posted(db_path: str, asins: list[str]) -> None:
    if not asins:
        return
    ph = "%s" if USE_POSTGRES else "?"
    now = __import__("datetime").datetime.now(__import__("datetime").timezone.utc).isoformat()
    ...
    with _get_conn(db_path) as conn:
        if USE_POSTGRES:
            conn.cursor().execute(
                f"UPDATE products SET reddit_posted_at={ph} WHERE asin IN ({placeholders})",
                (now, *asins))
        else:
            conn.execute(
                f"UPDATE products SET reddit_posted_at=? WHERE asin IN ({placeholders})",
                (now, *asins))
